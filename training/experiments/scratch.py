...
from training.common import SegmentationLoss, SimpleLogCallback, prepare_data_for_segmentation_loss
...
with tf.device("CPU"):
    ...
    model.make_model()
    image = np.reshape(train_dataset[0][0][:100, :, :], (100, *train_dataset[0][0][0, :, :].shape))

    y_true = train_dataset[0][1][:100, :, :]
    y_pred = model(image)
